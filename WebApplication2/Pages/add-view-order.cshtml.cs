...
{
    public class AddAndViewOrderModel : PageModel
    {
        ...
        public AddAndViewOrderModel(UserManager<ApplicationUser> userManager)
        {
        ...
        private async Task<List<CartItem>> FetchCartItemFromDatabase() // Changed return type to async Task<List<CartItem>>
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(_connectionString))
                {
                    ...
                    string sql = "SELECT TOP (SELECT COUNT(*) FROM Cart WHERE UserName = @UserName) OD.orderID, P.productID, PH.Image, P.productName, OD.productQuantity, OD.productPrice, OD.productSubTotal, OD.orderTotal, OD.UserName, OD.OrderStatus, OD.orderDate FROM OrderDetails OD LEFT JOIN Products P ON OD.productID = P.productID LEFT JOIN Photos PH ON P.productID = PH.productID WHERE OD.UserName = @UserName ORDER BY OD.orderDate DESC";
